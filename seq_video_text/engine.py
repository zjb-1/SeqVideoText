...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, iter_param_save_path,
                    max_norm: float = 0, iter_param_save_step=-1):
    model.train()
    criterion.train()
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        # for bug_t in targets:
        #     bug_v_id = bug_t["video_id"].item()
        #     bug_img_ids = bug_t["sample_indx"].tolist()
        #     print([data_loader.dataset.vid_infos[bug_v_id]["file_names"][i] for i in bug_img_ids])
        ...
        epoch_iter_count += 1
        iter_all_count = epoch * len(data_loader) + epoch_iter_count
        if iter_param_save_step >= 0 and iter_all_count % iter_param_save_step == 0:
            utils.save_on_master({
                'model': model.module.state_dict(),
                'epoch': epoch,
            }, iter_param_save_path / f'checkpoint_step{iter_all_count}.pth')
