# -*- coding: utf-8 -*-
# https://github.com/hikopensource/DAVAR-Lab-OCR/tree/main/demo/videotext/evaluation
import os
import json
import time
...
from eval_tools import evaluate_utils
...
def MOT(track_dict, gt_dict, voca_dict, care_rcg):
    ...
    print('all frames FN: ' + str(total_falseneg))
    print('all frames FP: ' + str(total_falsepos))
    print('all frames IDsw: ' + str(total_idswitch))
    print('all frames total_gt: ' + str(total_gt))
    print('all frames iou: ' + str(total_iou))
    print('all frames TP: ' + str(total_truepos))
    print("************************************************************************************************\n")

    return TOTAL_MOTA, TOTAL_MOTP


def eval_mot(predict_file, gt_file, voca_file=None, care_rcg=0):
    start_time = time.time()

    # Define predict file
    with open(predict_file, 'r') as p_f:
        track_res = json.load(p_f)

    # Define gt
    with open(gt_file, 'r') as g_f:
        gt_res = json.load(g_f)

    if voca_file is not None:
        with open(voca_file, 'r') as v_f:
            voca_res = json.load(v_f)
    else:
        voca_res = None

    # MOT evaluation
    res = MOT(track_res, gt_res, voca_res, care_rcg)
