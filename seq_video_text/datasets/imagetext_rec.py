...
class CocoDetection(TvCocoDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks,
                 with_rec=False, cache_mode=False, local_rank=0, local_size=1):
...
def convert_coco_poly_to_mask(segmentations, height, width):
    ...
    for polygons in segmentations:
        rles = coco_mask.frPyObjects(polygons[0], height, width)
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target):
        ...
        # recognize
        rec = None
        if anno and self.with_rec:
            assert 'rec' in anno[0], "annotations don't contain the att: rec"
            rec = [obj['rec'] for obj in anno]
            rec = torch.as_tensor(rec, dtype=torch.int64)
...
def make_coco_transforms(image_set):
    ...
    scales = [544, 608, 672, 736, 800, 864, 928, 960, 1024]

    if image_set == 'train':
        return T.Compose([
            # T.RandomHorizontalFlip(),
            T.RandomSelect(
                T.RandomResize(scales, max_size=1600),
                T.Compose([
                    # T.RandomResize([400, 500, 600]),
                    T.RandomSizeCropwithInstance(0.5, 1.0, crop_instance=False),
                    T.RandomResize(scales, max_size=1600),
                ])
            ),
            # T.Test_Show(save_root="/lustre/home/jbzhang/SeqFormer/crop_debug/", show_mask=True),
            normalize,
        ])
...
def build(image_set, args, ds_ind=0):
    ...
    dataset = CocoDetection(img_folder, ann_file, transforms=make_coco_transforms(image_set), return_masks=args.masks,
                            with_rec=args.rec, cache_mode=args.cache_mode,
