...
class YTVOSDataset:
    def __init__(self, img_folder, ann_file, transforms, return_masks, num_frames, dataset_file="ytvos"):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            # global sampling
            if num_frames > 3:    # TODO: sample not ok
                all_inds = list(range(vid_len))
                global_inds = all_inds[:min(sample_indx)]+all_inds[max(sample_indx):]
                global_n = num_frames - len(sample_indx)  #  2
            ...
            zjb_time0 = time.time()         ###########
            print("---------------------")
            print("pre index: {:.5f}".format(zjb_time0 - zjb_time1))        ############
            ...
            zjb_time2 = time.time()         ###########
            print("pre imgs: {:.5f}".format(zjb_time2 - zjb_time0))        ############
            ...
            zjb_time3 = time.time()         ###########
            print("pre target: {:.5f}".format(zjb_time3 - zjb_time2))        ############

            if self._transforms is not None:
                img, target = self._transforms(img, target, num_frames)

            zjb_time4 = time.time()         ###########
            print("trans: {:.5f}".format(zjb_time4 - zjb_time3))        ############
            print("---------------------")
...
def convert_coco_poly_to_mask(segmentations, height, width, is_crowd, dataset_file="ytvos"):
    masks = []
    for i, seg in enumerate(segmentations):
        if seg == 'None' or seg is None:
            ...
            if not is_crowd[i]:
                if dataset_file == 'VideoText':
                    seg = coco_mask.frPyObjects(seg, height, width)   # revised _mask.pyx
                else:
...
class ConvertCocoPolysToMask(object):
    def __init__(self, return_masks=False, dataset_file="ytvos"):
        self.return_masks = return_masks
    ...
    def __call__(self, image, target, target_inds, sample_inds):
        ...
        if self.return_masks:
            masks = convert_coco_poly_to_mask(segmentations, h, w, iscrowd, self.dataset_file)
        target = {}
        target["boxes"] = boxes
...
def build(image_set, args):
    ...
    if args.dataset_file == "VideoText":
        assert args.train_img_prefix != None and args.train_ann_file != None and args.test_img_prefix != None \
                and args.test_ann_file != None, 'please provide the file path'
        PATHS = {
            "train": (root / args.train_img_prefix, root / args.train_ann_file),
            "val": (root / args.test_img_prefix, root / args.test_ann_file),
        }
        img_folder, ann_file = PATHS[image_set]
        print('use VideoText dataset')
        dataset = YTVOSDataset(img_folder, ann_file, transforms=make_coco_transforms(image_set), return_masks=args.masks, num_frames = args.num_frames, dataset_file=args.dataset_file)
    
    elif args.dataset_file == 'YoutubeVIS' or args.dataset_file == 'jointcoco':
        mode = 'instances'
        PATHS = {
            "train": (root / "train/JPEGImages", root / "annotations" / 'train.json'),
            "val": (root / "valid/JPEGImages", root / "annotations" / 'valid.json'),
        }
        img_folder, ann_file = PATHS[image_set]
