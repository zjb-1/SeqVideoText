        ...
        if test_mode:  # build test dataset
            ...
            if "BOVText" in self.img_folder.__reduce__()[-1]:
                ...
                with open("/lustre/datasharing/jbzhang/data/rt_1k/rt1k_img_ids.pkl", 'rb') as f:
                    self.img_ids = pickle.load(f)
            # elif "DSText" in self.img_folder.__reduce__()[-1]:
            #     with open("/lustre/datasharing/jbzhang/data/DSText/dstext_img_ids.pkl", 'rb') as f:
            #         self.img_ids = pickle.load(f)
            else:
                self.img_ids = []
                for idx, vid_info in enumerate(self.vid_infos):
...
def make_coco_transforms_vt(image_set, scales, max_size):
    normalize = T.Compose([
        T.ToTensor(),
    ...
    if image_set == 'train':
        return T.Compose([
            ...
            # T.RandomResize([608, 640, 672, 736, 768, 800], 1333),     # [608, 640, 672, 736, 768], 1333
            # T.RandomResize([448, 480, 512], 960),  # [608, 640, 672, 736, 768], 1333    BOVText dataset
...
def build(image_set, args, ds_ind=0):
    ...
    if args.dataset_name_need_small_scale is None or args.train_img_prefix[ds_ind].split("/")[0] not in args.dataset_name_need_small_scale:
        scales = [608, 640, 672, 736, 768]
        max_size = 1333
    else:
        scales = [448, 480, 512]
        max_size = 960

    dataset = VideoTextDataset(img_folder, ann_file, transforms=make_coco_transforms_vt(image_set, scales, max_size),
