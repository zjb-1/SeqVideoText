...
def main(args):
    print("inference_imagetext2_rec")
    ...
    with torch.no_grad():
        ...
        for i in range(img_num):
            ...
            im = Image.open(os.path.join(folder, file_name)).convert('RGB')
            ...
            PostProcess(folder, [file_name], i, i, saved_boxes, saved_masks, saved_recs,
                        ...
                        submit_mode="mask", nms_iou=-1, draw_masks=True)
...
def draw_text(img, bbox, irec):
    ...
    if text != ' ':
        ...
        img = cv2.putText(img, text, text_pos, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1, cv2.LINE_AA,)

    return img


def PostProcess_show(folder, file_names, frame_index_s, pred_bboxes: np.ndarray, save_root,
                     pred_masks=None, pred_recs=None, draw_masks=False):
    """
    ...
    for i in range(frames_num):
        ...
        for b in range(frame_boxes.shape[0]):
            ...
            if pred_recs is not None:
                img = draw_text(img, bbox, pred_recs[i][b])
...
def PostProcess(folder,
    ...
    for i in range(frames_num):
        ...
        if submit_path is not None:
            ...
            # txt_name = 'res_img_' + file_names[i].split("_")[-1].replace('.jpg', '.txt')  # revise.for.icdar2017
            ...
            if submit_mode == "det":
                ...
                for mask_i in range(nms_masks.shape[0]):
                    ...
                    if quad is None:
                        continue
                    s = ",".join(map(str, quad)) + "\n"
                    # s = ",".join(map(str, quad)) + "," + str(0.95) + "\n"  # revise.for.icdar2017
                    with open(txt_path, 'a') as f:
                        f.write(s)

        if save_root is not None:
            PostProcess_show(folder, file_names, frame_index_s + i, nms_boxes, save_root, nms_masks, nms_recs, draw_masks=draw_masks)
