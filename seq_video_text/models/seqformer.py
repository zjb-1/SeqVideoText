...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_frames, num_queries, num_feature_levels,
        ...
        self.obj_embed = nn.Linear(hidden_dim, 1)  # used to pred the confidence of the boxes
        ...
        nn.init.normal_(self.obj_embed.weight, 0, 0.01)
        nn.init.constant_(self.obj_embed.bias, 0)
        ...
        if with_box_refine:
            ...
            self.obj_embed = _get_clones(self.obj_embed, num_pred)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)])  # 按 J的顺序调换一下 label
        ...
        labels = torch.zeros_like(src_logits)  # [bs * query_num, class_num]
        labels[pos_inds, target_classes[pos_inds]] = 1

        # target_classes_onehot = torch.zeros([src_logits.shape[0], src_logits.shape[1], src_logits.shape[2] + 1],
        #                                     dtype=src_logits.dtype, layout=src_logits.layout, device=src_logits.device)
        # target_classes_onehot.scatter_(2, target_classes.unsqueeze(-1), 1)  # 把1 在dim=2下，按targe_classes位置填到onehot中
        #
        # target_classes_onehot = target_classes_onehot[:, :, :-1]  #
        # ## last_dim = num_classes
        loss_ce = sigmoid_focal_loss(src_logits, labels, num_boxes, alpha=self.focal_alpha, gamma=2) * src_logits.shape[1]
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        src_boxes = outputs['pred_boxes'].permute(0, 2, 1, 3)[idx]  # [\sum{selected_inst}, nf, 4]
        num_insts, nf = src_boxes.shape[:2]
        target_boxes = torch.cat([v["boxes"].reshape(-1, nf, 4)[i] for (_, i), v in zip(indices, targets)])
        assert target_boxes.shape[0] == src_boxes.shape[0], "target_boxes deal error"
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        if type(src_masks) == list:
            src_masks = torch.cat(src_masks, dim=1)[0]    # [\sum{num_inst}, num_frames, H/4, W/4]
    ...
    def loss_rec(self, outputs, targets, indices, num_boxes):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
    ...
    def loss_obj(self, outputs, targets, indices, num_boxes):
        assert 'pred_objs' in outputs
        idx = self._get_src_permutation_idx(indices)
        pred_objs = outputs['pred_objs']  # [bs, nf, num_querries, 1]
        pred_objs = pred_objs.transpose(1, 2)   # [bs, num_querry, nf, 1]
        nf = pred_objs.shape[2]

        target_objs = torch.cat([v["valid"].reshape(-1, nf, 1)[i] for (_, i), v in zip(indices, targets)]).float()

        labels = torch.zeros_like(pred_objs, dtype=torch.float)
        labels[idx] = target_objs

        pred_objs = pred_objs.flatten(0, 1).flatten(1)  # [bs * num_querry, nf * 1]
        labels = labels.flatten(0, 1).flatten(1)
        loss_obj = sigmoid_focal_loss(pred_objs, labels, num_boxes) * pred_objs.shape[1]
    ...
    def forward(self, outputs, targets, indices_list, valid_ratios):
        ...
        torch.cuda.empty_cache()  # add.zjb
...
def build(args):
    ...
    #     num_classes = 42
    # if args.dataset_file == 'VideoText' or args.dataset_file == 'SynthText' or args.dataset_file == 'ImageText':
    #     num_classes = 1  # 2
    ...
    if args.masks:
        model = SeqFormer(args, model, freeze_detr=False, rel_coord=args.rel_coord)
    ...
    if args.obj:
        weight_dict["loss_obj"] = args.obj_loss_coef
    # TODO this is a hack
    if args.aux_loss:
        aux_weight_dict = {}
