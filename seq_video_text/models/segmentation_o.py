...
class SeqFormer(nn.Module):
    ...
    def extra_rec_feat(self, indices, outputs, gt_targets, enc_memory, spatial_shapes):
        gt_masks = list()
        gt_boxes = list()
        ...
        feats = enc_memory.transpose(0, 1)  # [nf, bs, \sum_lvl{h * w}, channel]
        feats = feats.reshape(-1, *feats.shape[-2:])  # [nf * bs, \sum_lvl{h * w}, channel]
