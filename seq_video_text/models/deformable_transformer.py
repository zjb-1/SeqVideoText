...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        src_flatten = torch.cat(src_flatten, 2)   # [N, nf, sum(H*W), C]
        mask_flatten = torch.cat(mask_flatten, 2)  # [N, nf, sum(H*W)]
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 2)
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=src_flatten.device)
        level_start_index = torch.cat((spatial_shapes.new_zeros((1, )), spatial_shapes.prod(1).cumsum(0)[:-1]))  # 把 src_flatten 按feat level切分的index
        valid_ratios = torch.stack([self.get_valid_ratio(m[:, 0]) for m in masks], 1)  # 真实图片h w有效区域率
        ...
        query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1)  # query_embed: pos embedding
        tgt = tgt.unsqueeze(0).expand(bs, -1, -1)
        reference_points = self.reference_points(query_embed).sigmoid()
        reference_points = reference_points.unsqueeze(1).repeat(1, nf, 1, 1)    # [bs,nf,300,2]
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_box, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        ...
        # self attention for mask&class query (tgt)
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        # src: [bs, nf, len_q, 256] encoder output
